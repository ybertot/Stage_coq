...
have gl : left_pt g1 = p by rewrite outs // inE eqxx.
...
+ move=> g gin; have := noc _ _ (oesub _ gin) lein.
  move=>/no_crossingE[]; first by rewrite lefts // sval.
  by rewrite lefts // => _ /(_ pl).
+ move=> g gin; have := noc _ _ (oesub _ gin) hein.
  move=>/no_crossingE[]; first by rewrite lefts // sval.
  by rewrite lefts // => /(_ ph).
exact: (sub_in2 oesub).
Qed.
...
(* TODO : redo this proof using closing_rest_ind *)
have inrest : forall CC c, c \in CC -> inside_open_cell q c ->
  ...
  rewrite inE=> /orP[/eqP cisc' | cincc'] inq sval'.
    rewrite /=; case: cc' Ih sval' => [ | c2 cc'2] Ih sval'.
      ...
      move: liml; rewrite /right_limit cisc' => ->.
      by  case: ifP=> _ /=; rewrite p2e.
...
have inclosing : forall c, c \in cc -> inside_open_cell q c ->
  ...
  rewrite inE=> /orP[/eqP cisc' | cincc'] inq sval'.
    rewrite /closing_cells; case: cc' sval' => [ | c2 cc'2] sval'.
      ...
      case/andP: inq=> ctn /andP[liml _] /=.
      move: ctn; rewrite /contains_point /c'' /= cisc' => ->.
      by move: liml; rewrite /right_limit cisc' (no_dup_seq_x3 _ vp2) => ->.
    have /exists_point_valid [p1 vp1] : valid_edge (low c') (point e).
      by have := sval' c'; rewrite inE eqxx => /(_ isT)/andP[].
...
have [v1 on1 x1] : [/\ valid_edge le p1, p1 === le & p_x e = p_x p1].
  have [on1 xp] := intersection_on_edge vip1; split => //.
  rewrite /valid_edge -xp; exact vl.
have eong : e === g by rewrite -(lg g) ?inE ?eqxx // left_on_edge.
...
Lemma no_overlap_eC (c1 c2 : cell) :
  no_overlap_e c1 c2 -> no_overlap_e c2 c1.
Proof.
move=> [-> // |]; first by left.
...
suff main : forall c, c \in opening_cells p oe og he -> no_overlap_e W c.
  ...
  - by rewrite c2W; apply/no_overlap_eC/main.
  apply: (Ih _ _ og) => //.
...
have eh : e === high c.
  ...
  apply/andP; split; [apply/eqP/le_anti; apply/andP; split;[exact: under|]|].
    rewrite leNgt; exact: above.
  by move: sval=> /andP[] _ /andP[] /andP[].
...
have eloc1 : e === low c1.
  ...
  by apply/le_anti; apply/andP; split;[apply: eh |rewrite leNgt; apply: eh' ].
...
- move=> /(_ erefl) eh; move: ctp=> /andP[] el eh' /=.
  have {}eh' : e === high c.
    apply/andP; split;[apply/eqP | ].
      by apply/le_anti/andP; split;[apply: eh' | rewrite leNgt eh].
    by move: o2=>/andP[] _; rewrite /valid_edge x2.
  case: ifP=>[/eqP q1| enp1]; move: (o1);
  rewrite /=  -?q1 -?q2 -?x1 -?x2 ?eqxx ?eh'/= => ->; rewrite ?andbT //.
  by rewrite (above_edge_strict_higher_y x1 (negbT enp1) el).
- move=> /(_ erefl) [el eh]; move: ctp=> /andP[] el' eh' /=.
  have {}eh' : e === high c.
    apply/andP; split;[apply/eqP | ].
      by apply/le_anti/andP; split;[apply: eh' | rewrite leNgt eh].
    by move: o2=>/andP[] _; rewrite /valid_edge x2.
  have {}el' : e === low c.
    apply/andP; split;[apply/eqP | ].
      by apply/le_anti/andP; split;[apply: el | rewrite leNgt el'].
    by move: o1=>/andP[] _; rewrite /valid_edge x1.
  by rewrite el' eh' eqxx.  
move=> /(_ erefl) el; move: ctp => /andP[] el' eh.
have {}el' : e === low c.
  apply/andP; split;[apply/eqP | ].
    by apply/le_anti/andP; split;[apply: el | rewrite leNgt el'].
  by move: o1=>/andP[] _; rewrite /valid_edge x1.
...
have [large | small] := boolP (1 < size cc)%N.
 ...
 have lin : last dummy_cell cc \in cc.
  ...
  have eabovehd : e >>= high (head dummy_cell cc).
    by move: adj ctps; rewrite ccdq /=; case subq : (cutlast _) => [ | a q'];
      move=> /= /andP[]/eqP -> _ /andP[] _ /andP[] /andP[].
  have ebelowtl : e <<= low (last dummy_cell cc).
    move: adj; rewrite [X in adjacent_cells X]ccdq /= adj_aux_path -cats1.
    rewrite cat_path /= => /andP[] _ /andP[] /eqP <- _.
    elim/last_ind: {2}(cutlast (behead cc)) (erefl (cutlast (behead cc))) =>
       [-> /= | a b _ /[dup]ctq ->].
      by have := (allP ctps _ hin)=> /andP[].
    rewrite last_rcons (proj2 (andP (allP ctps b _))) //.
    by rewrite behead_subset // cutlast_subset // ctq mem_rcons inE eqxx.
  ...
  have [el eh] : e <<= low c' /\ e >>= high c'.
    move: (cin) (ccdq); case/splitPr => s1 s2 => ccdq'.
    have [s1sub s2sub] : {subset s1 <= cc} /\ {subset s2 <= cc}.
      by split; move=> x X; rewrite ccdq' -!cats1 /= !(mem_cat, inE) ?X ?orbT.
    split.
      move: ccdq'; rewrite -cats1 -catA => ccdq'.
      move: adj; rewrite ccdq' /= adj_aux_path.
      rewrite cat_path /= => /andP[] _ /andP[] /eqP <- _.
      suff ls1in : last (head dummy_cell cc) s1 \in cc.
        by move: (allP ctps (last (head dummy_cell cc) s1) ls1in)=>/andP[].
      case s1eq : s1 => [ | x xs]; first exact: hin.
        by rewrite s1sub // s1eq /= mem_last.
    move: ccdq'; rewrite -cats1 /= -catA /=.
    rewrite -(cat1s c') (catA _ _ (s2 ++ _)) => ccdq'.
    move: adj; rewrite ccdq' /= adj_aux_path cat_path /= => /andP[] _.
    rewrite !cats1 last_rcons.
    case s2eq : s2 => [ | x xs] /andP[] /eqP -> _ //=.
      by have/andP[] := allP ctps _ lin.
    suff /(allP ctps)/andP[] : x \in cc by [].
    by rewrite ccdq' /= s2eq !(mem_cat, inE) eqxx !orbT.
  have c'in2 : c' \in cc by rewrite behead_subset // cutlast_subset.
  apply: close_cell_ok => //.
...
Lemma closing_rest_subset_contact p cc c :
 ...
 all closed_cell_side_limit_ok (closing_rest p cc) ->
 exists2 c1, c1 \in cc & strict_inside_open p c1.
Proof.
pattern cc, (closing_rest p cc); apply: (@closing_rest_ind p) => //.
  move=> c0 p1 vip; rewrite inE =>/eqP -> /=.
  move=> /andP[] /= lh /=.
  move=> /andP[] ll rl /andP[] /closed_right_imp_open cok _.
  exists c0; first by rewrite inE eqxx.
  by rewrite /strict_inside_open lh ll /= (lt_le_trans rl cok).
move=> c0 a q Ih; rewrite inE=>/orP[/eqP -> | ]; last first.
  move=> cin sic; rewrite allcons=> /andP[] _ allc.
  have [c1 c1in pc1] := Ih cin sic allc.
  by exists c1; rewrite // inE c1in orbT.
move=> pc0 /andP[] c0ok _; exists c0; first by rewrite inE eqxx.
move: pc0=> /andP[] /= lh /andP[] ll rl.
move: (closed_right_imp_open c0ok) => cok.
by rewrite /strict_inside_open lh ll /= (lt_le_trans rl cok).
Qed.
