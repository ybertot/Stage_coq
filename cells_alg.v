...
case: ifP=> [pxaway | /negbFE/eqP/[dup] pxhere /abovelstle palstol].
  ...
  have inv1 := (insert_opening_closeness close_fc clan close_lc).
  split.
    by move: inv1; rewrite /opening_cells oca_eq -cats1 -!catA.
  ...
  suff : seq_valid ((fc ++ ncs) ++ nlsto :: lc) p by [].
  apply/allP=> x; rewrite !(mem_cat, inE) -orbA => /orP[xf | ].
    have /andP [vlx vhx] := allP sval x (subfc _ xf).
    have := (allP inv1 x); rewrite mem_cat xf => /(_ isT) /andP claex.
    by rewrite (val_bet _ vlx) ?(val_bet _ vhx); case: claex.
  ...
  by rewrite (val_bet _ vlx) ?(val_bet _ vhx).
...
case: ifP=> [eabove | ebelow].
  case oe: (open_cells_decomposition _ _) => [[[[[fc' cc] lcc] lc] le] he].
  have [ocd' [lcc_ctn [allct [allnct [flcnct [heq [leq [lein hein]]]]]]]] :=
    open_cells_decomposition_main_properties oe (exi' eabove).
  have rfo' : s_right_form lop.
    apply/allP=> x xin; apply: (allP rfo).
    by  rewrite /open !(mem_cat, inE) xin !orbT.
  have sval' : seq_valid lop (point e).
    apply/allP=> x xin; apply: (allP sval).
    by rewrite /open !(mem_cat, inE) xin !orbT.
  have lopnnil : lop != [::].
    by apply/eqP=> abs; have [wc + _]:= exi' eabove; rewrite abs.
  have adj' : adjacent_cells lop.
    by move: adj; rewrite /open=> /adjacent_catW[] _ /= /path_sorted.
  have cbtom' : cells.cells_bottom_top (low (head dummy_cell lop)) top lop.
    rewrite /cells.cells_bottom_top/cells_low_e_top.
    rewrite lopnnil /= eqxx.
    have -> // : high (last dummy_cell lop) == top.
    move: cbtom; rewrite /open=> /andP[] _; rewrite last_cat.
    by elim /last_ind: (lop) lopnnil => [ | ? ? _]; rewrite //= !last_rcons.
  have lsthellop : lsthe = low (head dummy_cell lop).
    move: adj=> /adjacent_catW[] _ /=.
    by case: (lop) lopnnil => [ | ? ?] //= _ /andP[] /eqP <- _.
  have bet' : between_edges (low (head dummy_cell lop)) top (point e).
    rewrite /between_edges -lsthellop eabove.
    by move: inbox_e => /andP[] /andP[] _ -> _.
  have [pal puh vle vhe ncont] :=
    connect_properties cbtom' adj' rfo' sval' bet' ocd' allnct allct lcc_ctn
         flcnct.
  case oca_eq:(opening_cells_aux _ _ _ _) => [ncs nlsto] [] <- <- <- _ _ _ _.
  have dec_not_end :=
    fclc_not_end_aux cbtom' adj' rfo' sval' bet' ocd' allnct lcc_ctn
    flcnct erefl.
  have close_fc : close_alive_edges (fop ++ lsto :: fc') future_events.
    suff/head_not_end : close_alive_edges (fop ++ lsto :: fc')
                               (e :: future_events).
      apply=> c0; rewrite mem_cat => /orP[xinf | ].
        by apply: fop_note.
      rewrite inE => /orP[/eqP -> | ].
        apply: contrapositive_close_imp_cont.
        - by apply: (allP rfo).
        - by apply/andP; apply: (allP sval).
        by rewrite /contains_point -lstheq (negbTE eabove) andbF.
      by move=> cin; apply: dec_not_end; rewrite cin.
    apply/allP=> c0 cin; apply: (allP clae).
    move: cin; rewrite /open ocd' !(mem_cat, inE).
    by move=> /orP[-> | /orP[/eqP -> | -> ]]; rewrite ?eqxx ?orbT.
  have close_lc : close_alive_edges lc future_events.
    suff/head_not_end : close_alive_edges lc (e :: future_events).
      by apply=> c0 cin; apply: dec_not_end; rewrite cin orbT.
    apply/allP=> c0 cin; apply: (allP clae).
    by rewrite /open ocd' !(mem_cat, inE) cin !orbT.
  have endle : end_edge le future_events.
    suff  : end_edge le (e :: future_events).
      rewrite /end_edge leq; move=> /orP[-> // | ] /= /orP[ | ->]; last first.
        by rewrite orbT.
      by move: pal=> /[swap] /eqP <-; rewrite right_pt_below.
    have := (proj1 (andP (allP clae (head lcc cc) _))); rewrite leq; apply.
    rewrite /open ocd'.
    by case : (cc) => [ | ? ?]; rewrite !(mem_cat, inE) eqxx ?orbT.
  have endhe : end_edge he future_events.
    suff  : end_edge he (e :: future_events).
      rewrite /end_edge heq; move=> /orP[-> // | ] /= /orP[ | ->]; last first.
        by rewrite orbT.
      move: puh=> /[swap] /eqP <-; rewrite strict_nonAunder; last first.
        by apply: valid_edge_right.
      by rewrite right_on_edge.
    ...
    rewrite /open ocd'.
    by case : (cc) => [ | ? ?]; rewrite !(mem_cat, inE) eqxx ?orbT.
  move: endle endhe; rewrite leq heq => endle endhe.
  have clan := opening_cells_close vle vhe oute endle endhe cloe.
  have main := (insert_opening_closeness close_fc clan close_lc).
  split.
    by move: main; rewrite /opening_cells -leq -heq oca_eq -cats1 -!catA.
  have subf : {subset fop ++ lsto :: fc' <= open}.
    by move=> x xin; rewrite /open ocd' -2!cat_rcons catA mem_cat cat_rcons xin.
  have subl : {subset lc <= open}.
    by move=> x xin; rewrite /open ocd' !(mem_cat, inE) xin !orbT.
  suff : seq_valid (((fop ++ lsto :: fc') ++ ncs) ++ nlsto :: lc) p by [].
  apply/allP=> x; rewrite 2!mem_cat -orbA=> /orP[xold | ].
    have /andP [vlx vhx] := allP sval x (subf _ xold).
    have := (allP main x); rewrite mem_cat xold => /(_ isT) /andP claex.
    by rewrite (val_bet _ vlx) ?(val_bet _ vhx); case: claex.
  rewrite inE orbA=> /orP[ | xl]; last first.
    have /andP [vlx vhx] := allP sval x (subl _ xl).
    move: elexp;rewrite lexePt_eqVlt => /orP[/eqP <- | elexp'].
      by rewrite vlx vhx.
    have := (allP main x).
    rewrite !mem_cat xl !orbT => /(_ isT) /andP claex.
    by rewrite (val_bet _ vlx) ?(val_bet _ vhx); case: claex.
  move=> xin; have xin' : x \in opening_cells (point e) (outgoing e) le he.
    by rewrite /opening_cells oca_eq mem_rcons inE orbC.
  have := opening_valid oute vle vhe; rewrite -leq -heq in clan *.
  move=> /allP=> /(_ _ xin')/andP[vlx vhx].
  have [eelx eehx] := andP (allP clan _ xin').
  by rewrite (val_bet _ vlx) ?(val_bet _ vhx).
...
have ocd2 : open = (fop ++ fc') ++ cc ++ lcc :: lc.
  by rewrite /open ocd' catA.
have subf : {subset (fop ++ fc') <= open}.
  by move=> x xin; rewrite ocd2 mem_cat xin.
have adjf : adjacent_cells (fop ++ fc').
  by move: adj; rewrite ocd2=> /adjacent_catW[].
...
have [pal puh vl vh not_ct] := 
  connect_properties cbtom' adjl rfol svall bet ocd' all_nct all_ct
            lcc_ctn flcnct.
...
have clael : close_alive_edges lc (e :: future_events).
  apply/allP=> x xin.
  by apply: (allP clae); rewrite ocd2 !mem_cat inE xin !orbT.
have clael' : close_alive_edges lc future_events.
  ...
  have [puhlcc adj2] : point e <<< low (head dummy_cell lc) /\
                        adjacent_cells lc.
    move: adjl; rewrite ocd' lc_eq=> /adjacent_catW[] _ /adjacent_catW[] _ /=.
    by move=> /andP[] /eqP <- ->.
  have sub2 : {subset lc <= open}.
    by move=> x xin; rewrite mem_cat ocd' !(mem_cat, inE) xin !orbT.
...
case uoct_eq : (update_open_cell_top _ _ _) => [ncs lnc] [] <- <- <- _ _ _ _.
set open' := (fop ++ fc') ++ (rcons ncs lnc) ++ lc.
rewrite [X in is_true (_ X _) /\ is_true (_ X _)] (_ : _ = open'); last first.
  by rewrite /open' -cats1 -!catA.
have clae_part : close_alive_edges open' future_events.
  rewrite /close_alive_edges all_cat [all _ (fop ++ fc')]claef'.
  rewrite all_cat [all _ lc]clael' andbT andTb.
  rewrite -[all _ _]/(close_alive_edges _ _).
  have llsto_end : end_edge (low lsto) future_events.
    ...
    have fc'0 : fc' = [::].
      case fc_eq : (fc') => [// | c2 f'].
        move: ocd'; rewrite fc_eq /= => -[] lstoc2 _.
      have /all_nct/negP[] : c2 \in fc' by rewrite fc_eq inE eqxx.
      by apply: contains_point'W; rewrite -lstoc2.
  ...
  have he_end : end_edge he future_events.
    case lc_eq : lc => [ | c1 lc'].
      have hetop : he = top.
        ...
        by rewrite ocd2 lc_eq !last_cat -heq /= => /eqP.
  ...
  case out_eq : (outgoing _) => [ | g outs].
    by move=> [] <- <- /=; rewrite andbT; apply/andP.
  move=> oca_eq.
  have oc_eq : opening_cells (point e) (outgoing e) (low lsto) he =
                rcons ncs lnc.
    by rewrite /opening_cells out_eq oca_eq.
  rewrite -oc_eq.
  apply: opening_cells_close => //.
    by apply: (proj1 (andP (allP svall lsto _))); rewrite inE eqxx.
  by rewrite heq.
split=> //.
rewrite /seq_valid all_cat [X in _ && X]all_cat andbCA.
apply/andP; split; last first.
  rewrite -all_cat; apply/allP=> x xin.
  have /andP[vlx vhx] :
    valid_edge (low x) (point e) && valid_edge (high x) (point e).
    apply: (allP sval); rewrite ocd2 mem_cat [X in _ || X]mem_cat inE.
    by move: xin; rewrite mem_cat => /orP[] ->; rewrite ?orbT.
  have /andP[eelx eehx] :
     end_edge (low x) future_events && end_edge (high x) future_events.
    apply: (allP clae_part); rewrite /open' mem_cat [X in _ || X]mem_cat.
    by rewrite orbCA -mem_cat xin orbT.
  by rewrite !val_bet.
move: uoct_eq; rewrite /update_open_cell_top.
have vlo : valid_edge (low lsto) (point e).
  apply: (proj1 (andP (allP sval lsto _))).
  by rewrite /open mem_cat inE eqxx orbT.
...
case o_eq : (outgoing e) => [ | g s].
  move=> [] <- <- /=.
  by rewrite !val_bet.
move=> oca_eq.
have oc_eq : opening_cells (point e) (outgoing e) (low lsto) he = rcons ncs lnc.
  by rewrite /opening_cells o_eq oca_eq.
rewrite -oc_eq.
have /allP clan :=
  opening_cells_close vlo vhe oute eelo eehe (proj1 (andP cle)).
have /allP svaln := opening_valid oute vlo vhe.
apply/allP=> x /[dup] /clan /andP[eelx eehx] /svaln /andP[vlx vhx].
by rewrite !val_bet.
Qed.
(*
...
rewrite /opening_cells.
case oca_eq : (opening_cells_aux _ _ _ _) => [so co].
have op_new : opening_cells (point e) (outgoing e) low_e high_e = rcons so co.
...
have adjnew : adjacent_cells (rcons so co).
  by rewrite -op_new; apply: adjacent_opening'.
rewrite openeq in adjopen.
apply : (replacing_seq_adjacent c_nnil _ l_eq h_eq adjopen adjnew).
...
have newnnil := opening_cells_not_nil (outgoing e) low_e high_e (point e).
...
case : first_cells op_c_d op_dec newopnnil =>
   [/= |c1 q1 /=] op_c_d op_dec newopnnil; last first.
  by rewrite -openbottom op_dec /=.
case: (so) l_old => [ | a s'] /= <-; rewrite -openbottom op_dec /=;
by case : (contact_cells) c_nnil.
Qed.
...
have op_new : opening_cells (point e) (outgoing e) low_e high_e = rcons so co.
  by rewrite /opening_cells oca_eq.
have := higher_lower_equality outleft op_c_d op_new exi lowv highv.
rewrite last_rcons; move=> [] _ [] /= h_eq c_nnil.
case : last_cells op_c_d op_dec newopnnil => [/= op_c_d |c1 q1 _ op_dec /= _].
  rewrite !cats0 -opentop last_cat  => -> newn_nil /=.
  rewrite last_cat last_rcons.
  by case: contact_cells c_nnil h_eq {op_c_d} => [// | ? ?] /= _ ->.
by rewrite -opentop op_dec !last_cat /=.
Qed.
*)
...
have /allP rfn : s_right_form 
  ...
  apply: (opening_cells_right_form lev hev (underWC lP) hP _ oute) => //.
...
Lemma in_new_cell_not_in_first_old e open fc cc lcc lc le he:
...
have adj' : adjacent_cells result_open.
  ...
  - have := higher_edge_new_cells outs vle vhe.
    rewrite /opening_cells oca_eq => /(_ _ erefl) /eqP ->.
    by rewrite last_rcons heq.
  - by rewrite -cats1 -!catA -ocd.
  by have := adjacent_opening' vle vhe outs; rewrite /opening_cells oca_eq.
(*
...
have lfceq : high lfc = le.
  have := last_first_cells_high cbtom adj inbox_e oe; rewrite fceq.
...
have val' : all (fun g => @valid_edge R g (point e)) (s1 ++ [:: le, g2 & s2]).
  ...
  have := opening_cells_subset vle vhe outs => /(_ c).
  rewrite nceq => /(_ cin)=> /andP[] _.
  rewrite /= inE => /orP[/eqP -> //| /outs it].
...
have subeall : {subset outgoing e <= cell_edges open ++ outgoing e}.
  by move=> x xin; rewrite mem_cat xin orbT.
have noce : {in outgoing e &, no_crossing R}.
  by apply: (sub_in2 _ noc).
have lbh : le <| he.
  by apply: (open_cells_decomposition_low_under_high noco cbtom adj _ _ _ oe).
have headccin : head lcc cc \in open.
  by rewrite ocd !mem_cat; case: (cc) => [ | ? ?] /=; rewrite !inE eqxx ?orbT.
have leop : le \in cell_edges open by rewrite leq mem_cat map_f.
have heop : he \in cell_edges open.
  by rewrite !mem_cat heq map_f ?orbT // ocd !(mem_cat, inE) eqxx ?orbT.
have lein : le \in cell_edges open ++ outgoing e  by rewrite mem_cat leop.
have hein : he \in cell_edges open ++ outgoing e  by rewrite mem_cat heop.
have  [edgesabove edgesbelow nocout]:=
   outgoing_conditions pal puh lein hein vle vhe subeall noc outs.
...
have p'under : p' <<< g2.
  have := opening_cells_subset vle vhe outs; rewrite nceq => /(_ _ headin).
...
have c2ok : open_cell_side_limit_ok c2.
  ...
  have := opening_cells_side_limit vle vhe lebhe (underWC pal) puh noco' outs.
...
rewrite mem_cat.
have := opening_cells_subset vle vhe outs; rewrite nceq=> /(_ _ c2in) /andP[].
by rewrite inE => /orP[/eqP -> | -> ]; rewrite ?leop ?orbT.
Qed.
*)
Admitted.
...
have c1c2 : high c1 <| low c2.
  have /andP[_] := opening_cells_subset vle vhe outs c1in.
...
- move=> clc.
  ...
  by move: cmp2; rewrite -samey -samey' ltNge le_eqVlt cbev' orbT.
move=> /(opening_cells_subset vle vhe outs).
move=>/andP[] _; rewrite inE => /orP[/eqP ishe | ].
