...
Definition update_open_cell_top (c : cell) (new_high : edge) (e : event) :=
  if outgoing e is nil then
    let newptseq :=
(* when this is called, (point e) should alread be in the left points. *)
...
Definition inv1_seq (s : seq cell) :=
  ...
  (future_events = [::] \/
    seq_valid s (point (head (@dummy_event _) future_events))) /\
...
have [qright | qleft] := boolP(lexPt (point e) q).
  ...
  have [/andP[_ ->] /andP[_ ->]] : valid_cell c q.
    ...
    have := valid_between_events (lexPtW qright) limrq' vlce inbox_q end_edge_lc.
    have := valid_between_events (lexPtW qright) limrq' vhce inbox_q end_edge_hc.
    move=> vhcq vlcq.
    by split.
  by move: ctn=> /andP[] -> ->.
...
Record disjoint_general_position_invariant (bottom top : edge)
 (edge_set : seq edge)
 (s : scan_state) (ev : event) (future_events : seq event) :=
 { op_cl_dis :
     {in state_open_seq s & state_closed_seq s,
       disjoint_open_closed_cells R};
   cl_dis : {in state_closed_seq s &, disjoint_closed_cells R};
   inv1 : inv1_seq bottom top future_events (state_open_seq s);
   sval_fut : (future_events = [::]) \/
                seq_valid (state_open_seq s)
                   (point (head (@dummy_event _) future_events));
   lstx_eq : lst_x s = left_limit (lst_open s);
   high_lsto_eq : high (lst_open s) = lst_high s;
   edges_sub : {subset all_edges (state_open_seq s) future_events <=
                    bottom :: top :: edge_set};
   closed_events : close_edges_from_events future_events;
   out_events : {in future_events, forall e, out_left_event e};
   inbox_events : all (inside_box bottom top) [seq point x | x <- future_events];
   lex_future : sorted (@lexPtEv _) future_events;
   ...
   closed_at_left :
       {in state_closed_seq s & future_events,
...
Definition dummy_state :=
  {| sc_open1 := [::]; lst_open := dummy_cell; sc_open2 := [::];
     sc_closed := [::]; lst_closed := dummy_cell; lst_high := dummy_edge;
     lst_x := 0|}.
  ...
  | ev :: future_events =>
    let (nos, lno) :=
      opening_cells_aux (point ev) (sort (@edge_below _) (outgoing ev))
           bottom top in
      {| sc_open1 := nos; lst_open := lno; sc_open2 := [::];
         sc_closed := [::]; 
         lst_closed := close_cell (point ev) (start_open_cell bottom top);
         lst_high := top;
         lst_x := p_x (point ev) |}
  end.
...
rewrite /start.
case evsq : events => [ | ev future_events]; first by [].
move=> _.
have evins : ev \in events by rewrite evsq inE eqxx.
rewrite /initial_state.
have [op0 op0q] : exists op0, op0 = start_open_cell bottom top.
  by exists (start_open_cell bottom top).
case oca_eq:  (opening_cells_aux _ _ _ _) => [nos lno].
...
have oks0 : all open_cell_side_limit_ok ops0.
  by move: startok; rewrite /ops0 op0q /= andbT.
...
have cl0q : cl0 = close_cell (point ev) op0 by rewrite /cl0 st1q op0q.
rewrite -cl0q in rf1 adj1 cbtom1 clae1 op1_cl1_dis.
...
have : state1 = {| sc_open1 := nos; lst_open := lno; sc_open2 := [::];
        sc_closed := [::]; lst_closed := cl0; lst_high := he1; lst_x := lstx1 |}.
  by rewrite /cl0 st1q /= he1q lstx1q.
...
have evin' : {in events, forall e, inside_box bottom top (point e)}.
...
have rl_closed1 : {in [:: cl0] & future_events,
  forall c e, right_limit c <= p_x (point e)}.
...
rewrite /=.
move=> dis_open_closed dis_cl inv1 sval' lstxq lstheq edges_sub.
move=> /[dup] cle /andP[cl_e_fut' cle'] out_fut'.
move=> /[dup]  inbox_all_events' /andP[inbox_e inbox_all_events].
move=> lexevs edges_pairwise oks cl_at_left.
move=> /andP[] /andP[] lstxlte lstx_fut' ltfut'.
move: (inv1)=> [] clae [] pre_sval [] adj [] cbtom rfo.
have {sval'}sval : seq_valid (fop ++ lsto :: lop) (point ev') by case: sval'.
...
case oca_eq : (opening_cells_aux _ _ _ _) => [nos lno].
apply: Ih.
have oute_rec : out_left_event ev' by apply: out_fut'; rewrite inE eqxx.
have noc : {in all_edges (fop ++ lsto :: lop) (ev' :: fut') &,
                 no_crossing R}.
  by move=> g1 gt2 g1in g2in; apply: nocs; apply: edges_sub.
have := invariant1_default_case inbox_all_events' oute_rec rfo cbtom adj
            sval cle clae noc lexevs.
  rewrite oe oca_eq=> /[dup] inv2 -[] clae' [] sval' [] adj' []cbtom' rfo'.
have inbox_e' : inside_box bottom top (point ev') by exact inbox_e.
...
have right_limit_ev' : 
  ...
  by move=> c cin; apply: cl_at_left; rewrite ?mem_rcons // inE eqxx.
have cl_at_left' : {in cls', forall c, right_limit c <= p_x (point ev')}.
  by move=> c cin; apply: cl_at_left; rewrite // inE eqxx.
have := step_keeps_disjoint_default inbox_all_events' oute_rec rfo
         cbtom adj sval edges_pairwise oks dis_open_closed dis_cl cl_at_left'.
...
have oks' : all open_cell_side_limit_ok ((fc ++ nos) ++ lno :: lc).
  have := step_keeps_open_side_limit_default inbox_all_events' oute_rec rfo
    cbtom adj sval oks; rewrite oe oca_eq.
  by [].
have /esym left_last : left_limit lno = p_x (point ev').
  apply: (opening_cells_left oute_rec vl vp).
  by rewrite /opening_cells oca_eq mem_rcons inE eqxx.
...
have sub_all' : {subset all_edges ((fc ++ nos) ++ lno :: lc) fut' <=
                     [:: bottom, top & s]}.
  have := step_keeps_subset_default inbox_all_events' oute_rec
            rfo cbtom adj sval.
  rewrite oe oca_eq !catA /= /all_edges => main g.
  rewrite mem_cat=> /orP[ | gin]; last first.
    apply: edges_sub; rewrite mem_cat; apply/orP; right.
...
have out_evs' : {in fut', forall e, out_left_event e}.
  by move=> e ein; apply: out_fut'; rewrite inE ein orbT.
have gen_case' : all (fun e => p_x (point ev') <  p_x (point e)) fut' &&
                  sorted (fun e1 e2 => p_x (point e1) < p_x (point e2)) fut'.
  by move: ltfut'; rewrite path_sortedE; last (move=> x y z; apply: lt_trans).
...
have lexevs' : sorted (@lexPtEv _) fut' by apply: (path_sorted lexevs).
...
Record edge_covered_general_position_invariant (bottom top : edge)
 (edge_set covered_set : seq edge)
 (s : scan_state) (future_events : seq event) :=
 { inv1_ec : inv1_seq bottom top future_events (state_open_seq s);
   edges_sub_ec : {subset all_edges (state_open_seq s) future_events <=
                    bottom :: top :: edge_set};
    non_in_ec : 
      {in edge_set & future_events, forall g e, non_inner g (point e)};
   out_events_ec : {in future_events, forall e, out_left_event e};
   inbox_events_ec : 
       all (inside_box bottom top) [seq point x | x <- future_events];
   sides_ok_ec : all open_cell_side_limit_ok (state_open_seq s);
   cle_ec : close_edges_from_events future_events;
   lexev_ec : sorted (@lexPtEv _) future_events;
   uniq_ec : {in future_events, forall e, uniq (outgoing e)};
   ...
   general_pos_ec :
     all (fun ev => lst_x s < p_x (point ev)) future_events &&
     sorted (fun e1 e2 => p_x (point e1) < p_x (point e2)) future_events;
   edge_covered_ec : {in covered_set, forall g,
     edge_covered g (state_open_seq s) (state_closed_seq s)}
   }.
...
case vpq: (vertical_intersection_point _ _) => [ the_point /= | ]; last by [].
  move=> _.
  have [_ xpoint]:= intersection_on_edge vpq.
  rewrite /lexPt; apply/orP; left; rewrite -xpoint.
  by move: inbox_p=> /andP[] _ /andP[] _ /andP[].
case vpq : (vertical_intersection_point _ _) => [ some_point /= | ]; last by [].
  move=> _.  
...
have sval0 : seq_valid [:: start_open_cell bottom top] (point e).
  move: inbox_e=> /andP[] _ /andP[] /andP[] a b /andP[] c d.
  by rewrite /= /valid_edge !ltW.
have /andP [vb vt] : valid_edge bottom (point e) && valid_edge top (point e).
  by apply: (allP (sval0) (start_open_cell bottom top)); rewrite inE eqxx.
have cle0 : close_edges_from_events (e :: evs) by rewrite -evsq.
have cle1 : close_edges_from_events evs by move: cle0=> /andP[].
have clae0 : close_alive_edges bottom top [:: start_open_cell bottom top] 
                  (e :: evs).
  by rewrite -evsq /= /end_edge_ext !inE !eqxx ?orbT.
have sub0 : {subset all_edges [:: start_open_cell bottom top] (e :: evs) <= 
                bottom :: top :: s}.
  move=> g; rewrite mem_cat /events_to_edges /= => /orP[|gin].
    by move=> /orP[] /=; rewrite !inE ?orbF => ->; rewrite ?orbT.
  by rewrite !inE sub_es ?orbT // evsq.
have noc0 : {in all_edges [:: start_open_cell bottom top] (e :: evs) &, 
               no_crossing R}.
  by move=> g1 g2 /sub0 + /sub0 +; apply: nocs.
have lexevs0 : path (@lexPtEv _) e evs by move: lexev; rewrite evsq.
...
have inbox_es' : all (inside_box bottom top) [seq point e | e <- evs].
  by move: inbox_es; rewrite /= => /andP[].
have oe0 : open_cells_decomposition [:: start_open_cell bottom top] (point e) =
   ([::], [::], start_open_cell bottom top, [::], bottom, top).
  by apply: (open_cells_decomposition_single adj0 rf0 sval0 eab ebt).
have inv1 :
   inv1_seq bottom top evs (nos ++ [:: lno]).
  have := invariant1_default_case inbox_es oute rf0 cbtom0 isT sval0 cle0
            clae0 noc0 lexevs0.
  by rewrite cat0s oe0 oca_eq.
have [clae1 [sval' [adj1 [cbtom1 rf1]]]] := inv1.
have sub_e : {subset (outgoing e) <= s}.
  by move=> g gin; apply: sub_es; rewrite evsq /= mem_cat gin.
have sub1 : {subset all_edges (nos ++ [:: lno]) evs <= [:: bottom, top & s]}.
  move=> g; rewrite mem_cat=> /orP[| gin]; last first.
    by rewrite !inE sub_es ?orbT // evsq /= mem_cat gin orbT.
  rewrite in_cell_edges_has_cell=> /hasP[cg cgin /orP gq].
  have := opening_cells_subset vb vt oute; rewrite /opening_cells oca_eq.
  move=> /(_ cg); rewrite -cats1 !inE=> /(_ cgin) /andP[lin hin].
  move: gq=> [] /eqP ->.
    by move: lin=> /orP[-> | /sub_e ->]; rewrite ?orbT.
  by move: hin=> /orP[-> | /sub_e ->]; rewrite ?orbT.
have n_inner1 : {in s & evs, forall g e, non_inner g (point e)}.
  by move=> g ev gin evin; apply: n_inner; rewrite // evsq inE evin orbT.
have  out_es1 : {in evs, forall ev, out_left_event ev}.
  by move=> ev evin; apply: out_es; rewrite evsq inE evin orbT.
have sok1 :all open_cell_side_limit_ok (nos ++ [:: lno]).
  have:= step_keeps_open_side_limit_default inbox_es oute rf0 cbtom0 adj0 sval0.
  by move=> /=; rewrite andbT oe0 oca_eq=> /(_ startok).
have  btm_left_lex0 : 
...
have inj_high1 : {in nos ++ [:: lno] &, injective (@high _)}.
  ...
  have := step_keeps_injective_high_default inbox_es oute rf0 cbtom0 adj0 sval0.
  move=> /=; rewrite andbT oe0 oca_eq.
  by move=> /(_ startok uniq_e inj_high0 btm_left_lex0).
have btm_left_lex1 : {in nos ++ [:: lno] & evs,
          forall c e0, lexPt (bottom_left_corner c) (point e0)}.
  move=> c ev cin evin.
  have eev : lexPtEv e ev.
    move: lexevs0; rewrite path_sortedE; last by apply: lexPtEv_trans.
    by move=> /andP [] /allP + _; apply.
  ...
  by apply.
have gen_pos1 : all (fun ev => p_x (point e) < p_x (point ev)) evs &&
  sorted (fun e1 e2 => p_x (point e1) < p_x (point e2)) evs.
  move: gen_pos; rewrite evsq /= path_sortedE; first by [].
  by move=> x y z; apply lt_trans.  
...
have lexev1 : sorted (@lexPtEv _) evs by move: lexevs0=> /path_sorted.
have cov1 : {in outgoing e, forall g, (edge_covered g (nos ++ [:: lno])
            [:: close_cell (point e) (start_open_cell bottom top)])}.
  have := step_keeps_edge_covering_default inbox_es oute rf0 cbtom0 adj0 sval0
           _ inj_high0 btm_left_lex0 n_inner0 => /=.
  rewrite startok => /(_ isT [::] _ _ _ _ _ _ _ _ oe0 oca_eq).
  move=> main g gin.
  by apply: main; right.
constructor=> //.
Qed.
...
Lemma simple_step_edge_covered_general_position
  ...
  open_cells_decomposition (fop ++ lsto :: lop) (point ev) =
    (fc, cc, lcc, lc, le, he) ->
  edge_covered_general_position_invariant bottom top s
   cov_set (Bscan fop lsto lop cls lstc (high lsto) (left_limit lsto))
   (ev :: evs) ->
  edge_covered_general_position_invariant bottom top s
    (cov_set ++ outgoing ev) (simple_step fc cc lc lcc le he cls lstc ev)
    evs.
Proof.
move=> ltev boxwf nocs' inbox_s evin lexev evsub out_evs cle
  n_inner uniq_edges.
have nocs : {in bottom :: top :: s &, no_crossing R}.
  by apply: inter_at_ext_no_crossing.
set st := (Bscan fop lsto lop cls lstc (high lsto) (left_limit lsto)).
move=> oe.
move=> [] /[dup] inv_s [] clae [] []; first by [].
move=> /= sval [] adj [] cbtom rfo sub_edges {}n_inner out_es inbox_e sok.
move=> {}cle {}lexev uniq_evs inj_high.
move=> btm_left_lex /andP[] lstxlt pathlt covered.
...
have stq : st = Bscan fop lsto lop cls lstc (high lsto) (left_limit lsto).
  by [].
(* TODO: this should not be needed, if we had enough theorems about
  simple_step. *)
have lstxneq :  p_x (point ev) != left_limit lsto.
...
have inv_s' : inv1_seq bottom top evs (state_open_seq (step ev st)).
  have := invariant1_default_case inbox_e out_e rfo cbtom adj sval cle
    clae noc lexev.
  by rewrite /step /simple_step /= oe oca_eq lstxneq.
have := inv_s'=> -[].
rewrite /state_open_seq /= /step /simple_step lstxneq /=.
rewrite oe oca_eq /= => clae' [] sval' [] adj' [] cbtom' rfo'.
have sub_edges' : (* {subset all_edges ((fc ++ nos) ++ lno :: lc) evs <= [:: bottom, top & s]} *)
{subset all_edges (state_open_seq (step ev st)) evs <=
                         [:: bottom, top & s]} .
  have := step_keeps_subset_default inbox_e out_e rfo cbtom adj sval.
(*
  rewrite stq /= oe oca_eq=> main g.
*)
  rewrite /step /simple_step /= lstxneq oe oca_eq => main g.

  rewrite mem_cat=> /orP[ | gin]; last first.
    apply: sub_edges; rewrite mem_cat; apply/orP; right.
    by rewrite events_to_edges_cons mem_cat gin orbT.
  rewrite /state_open_seq/=.
  rewrite (cell_edges_sub_high cbtom' adj') -catA inE=> /orP[/eqP ->| /main].
    by rewrite inE eqxx.  
...
have out_evs' : {in evs, forall e, out_left_event e}.
  by move=> e ein; apply: out_es; rewrite inE ein orbT.
have inbox_e' : all (inside_box bottom top) [seq point x | x <- evs].
  by move: inbox_e=> /andP[].
have sok' : all open_cell_side_limit_ok (state_open_seq (step ev st)).
  have := step_keeps_open_side_limit_default inbox_e out_e rfo cbtom
       adj sval sok.
  by rewrite /step /simple_step /state_open_seq /= lstxneq oe oca_eq /=.
have uniq_ev : uniq (outgoing ev) by apply: uniq_evs; rewrite inE eqxx.
have inj_high' : {in state_open_seq (step ev st) &, injective (@high _)}.
  have := step_keeps_injective_high_default inbox_e out_e rfo cbtom adj sval
     sok uniq_ev inj_high btm_left_lex_e.
  by rewrite /step/simple_step/= oe oca_eq /state_open_seq lstxneq /= -catA.
have btm_left_lex' : {in state_open_seq (step ev st) & evs,
     forall c e, lexPt (bottom_left_corner c) (point e)}.
  have := step_keeps_btom_left_corners_default inbox_e out_e rfo cbtom adj
     sval noc btm_left_lex_e.
  rewrite /step /simple_step/= lstxneq /= oe oca_eq /= /state_open_seq /=.
  rewrite catA=> main.
  move=> c e cin ein; apply: main=> //=.
  move: lexev; rewrite path_sortedE; last by apply: lexPtEv_trans.  
  by move=> /andP[] /allP /(_ e ein).
have ltev' : all (fun e => lst_x (step ev st) < p_x (point e)) evs &&
       sorted (fun e1 e2 => p_x (point e1) < p_x (point e2)) evs.
  move : pathlt; rewrite /step /simple_step/= lstxneq oe oca_eq /=.
  by rewrite path_sortedE; last by move=> x y z; apply: lt_trans.
have n_inner2 : {in state_open_seq st, forall c, non_inner (high c) (point ev)}.
  move=> c cin.
...
have cov' : {in cov_set ++ outgoing ev,
  forall g, edge_covered g (state_open_seq (step ev st))
                  (state_closed_seq (step ev st))}.
  have := step_keeps_edge_covering_default
    inbox_e out_e rfo cbtom adj sval sok inj_high btm_left_lex_e n_inner2.
  rewrite /= /step /simple_step /= lstxneq.
  move=> /(_ _ _ _ _ _ _ _ _ _ oe oca_eq) => main.
  rewrite oe oca_eq /state_open_seq /state_closed_seq /=.
  have := main (state_closed_seq st) => {}main.
  move=> g gin.
  have /main : edge_covered g (fop ++ lsto :: lop) (state_closed_seq st) \/
         g \in outgoing ev.
    move: gin; rewrite mem_cat=> /orP[/covered|]; last by move=> -> ; right.
    by left.
  rewrite /state_open_seq /state_closed_seq /=.
  apply: edge_covered_sub.
    by rewrite -catA.
  by rewrite !cat_rcons /= -!cats1 -!catA /=.
rewrite [X in edge_covered_general_position_invariant _ _ _ _ X]
  (_ : _ = step ev st); last first.
  by rewrite /step /simple_step stq /= lstxneq oe oca_eq.
...
elim=> [ | ev evs Ih] op cl st cov_set.
  ...
  move=> inv1_o sub_edge _ _ _ sok _ _ uniq_evs inj_high _ _ it [] <- <-.
  move=> g; rewrite cats0=> /it.
  apply: edge_covered_sub; first by [].
  by move=> c; rewrite mem_rcons.
move=> inv0; rewrite events_to_edges_cons catA.
apply: Ih.
case stq : st => [fop lsto lop cls lstc lsthe lstx].
move: (inv0) => -[] _ _ _ _ _ _ _ _ _ _ _ /andP[] /andP[] + _.
rewrite lt_neqAle eq_sym => /andP[] lstxneq _ _ _.
rewrite /step; move: lstxneq; rewrite stq [lst_x _]/= => ->.
move: simple_step_edge_covered_general_position.
apply: simple_step_edge_covered_general_position.
apply: Ih.
have := simple_step_edge_covered_general_position_invariant
move=> [] /[dup] inv_s [] clae [] []; first by [].
move=> /= sval [] adj [] cbtom rfo sub_edges n_inner out_es inbox_e sok.
move=> cle lexev uniq_evs inj_high.
move=> btm_left_lex /andP[] lstxlt pathlt covered.
rewrite events_to_edges_cons catA.
apply: Ih.
have out_e : out_left_event ev by apply: out_es; rewrite inE eqxx.
have noc : {in all_edges (state_open_seq st) (ev :: evs) &, no_crossing R}.
  by move=> g1 g2 g1in g2in; apply: nocs; apply: sub_edges.
...
have lstxneq :  p_x (point ev) != lstx.
  by move: lstxlt; rewrite stq lt_neqAle eq_sym=> /andP[] /andP[].
case oe : (open_cells_decomposition (fop ++ lsto :: lop) (point ev)) =>
        [[[[[fc cc] lcc] lc] le] he].
case oca_eq : 
  (opening_cells_aux (point ev) (sort (@edge_below _) (outgoing ev)) le he) =>
      [nos lno].  
...
have inv_s' : inv1_seq bottom top evs (state_open_seq (step ev st)).
  have := invariant1_default_case inbox_e out_e rfo cbtom adj sval cle
    clae noc lexev.
  by rewrite /step /simple_step stq /= oe oca_eq lstxneq.
have := inv_s'=> -[].
rewrite /state_open_seq /= /step /simple_step stq lstxneq /=.
rewrite oe oca_eq /= => clae' [] sval' [] adj' [] cbtom' rfo'.
have sub_edges' : (* {subset all_edges ((fc ++ nos) ++ lno :: lc) evs <= [:: bottom, top & s]} *)
{subset all_edges (state_open_seq (step ev st)) evs <=
                         [:: bottom, top & s]} .
...
(*
  rewrite stq /= oe oca_eq=> main g.
*)
  rewrite /step /simple_step stq /= lstxneq oe oca_eq => main g.
  ...
  rewrite mem_cat=> /orP[] gin; apply: sub_edges; last first.
    by rewrite mem_cat events_to_edges_cons orbC mem_cat gin.
  by rewrite stq mem_cat mem_cat gin orbT.
have n_inner' : {in s & evs, forall g e, non_inner g (point e)}.
  by move=> g e gin ein; apply: n_inner; rewrite // inE ein orbT.
have out_evs' : {in evs, forall e, out_left_event e}.
  by move=> e ein; apply: out_es; rewrite inE ein orbT.
...
have sok' : all open_cell_side_limit_ok (state_open_seq (step ev st)).
  have := step_keeps_open_side_limit_default inbox_e out_e rfo cbtom
       adj sval sok.
  by rewrite /step /simple_step /state_open_seq stq /= lstxneq oe oca_eq /=.
have uniq_ev : uniq (outgoing ev) by apply: uniq_evs; rewrite inE eqxx.
have inj_high' : {in state_open_seq (step ev st) &, injective (@high _)}.
  have := step_keeps_injective_high_default inbox_e out_e rfo cbtom adj sval
     sok uniq_ev inj_high btm_left_lex_e.
  by rewrite /step /simple_step stq oe oca_eq lstxneq /state_open_seq /= -catA.
have btm_left_lex' : {in state_open_seq (step ev st) & evs,
     forall c e, lexPt (bottom_left_corner c) (point e)}.
  have := step_keeps_btom_left_corners_default inbox_e out_e rfo cbtom adj
     sval noc btm_left_lex_e.
  rewrite /step /simple_step stq lstxneq /= oe oca_eq /= /state_open_seq /=.
  rewrite catA=> main.
  move=> c e cin ein; apply: main=> //=.
  move: lexev; rewrite path_sortedE; last by apply: lexPtEv_trans.  
  by move=> /andP[] /allP /(_ e ein).
have ltev' : all (fun e => lst_x (step ev st) < p_x (point e)) evs &&
       sorted (fun e1 e2 => p_x (point e1) < p_x (point e2)) evs.
  move : pathlt; rewrite /step /simple_step stq lstxneq oe oca_eq /=.
  by rewrite path_sortedE; last by move=> x y z; apply: lt_trans.
have n_inner2 : {in state_open_seq st, forall c, non_inner (high c) (point ev)}.
  move=> c cin.
  have /sub_edges : high c \in all_edges (state_open_seq st) (ev :: evs).
...
have cov' : {in cov_set ++ outgoing ev,
  ...
  have := step_keeps_edge_covering_default
    inbox_e out_e rfo cbtom adj sval sok inj_high btm_left_lex_e n_inner2.
  rewrite stq /= /step /simple_step /= lstxneq.
  move=> /(_ _ _ _ _ _ _ _ _ _ oe oca_eq) => main.
  rewrite oe oca_eq /state_open_seq /state_closed_seq /=.
  have := main (state_closed_seq st) => {}main.
  move=> g gin.
  have /main : edge_covered g (fop ++ lsto :: lop) (state_closed_seq st) \/
    ...
    move: gin; rewrite mem_cat=> /orP[/covered|]; last by move=> -> ; right.
    by rewrite stq /=; left.
  rewrite stq /state_open_seq /state_closed_seq /=.
  apply: edge_covered_sub.
    by rewrite -catA.
  by rewrite !cat_rcons /= -!cats1 -!catA /=.
rewrite [X in edge_covered_general_position_invariant _ _ _ _ X]
