...
(* TODO : probably useless, thanks to the next few lemmas. *)
Lemma opening_cells_follow_safe_side_y le he pp:
  valid_edge le (point e) ->
  valid_edge he (point e) ->
  point e <<< he ->
  outgoing e != [::] ->
  p_x pp = p_x (point e) ->
  has (in_safe_side_left pp)
    (behead (opening_cells (point e) (outgoing e) le he)) =
    (p_y (point e) < p_y pp) && (pp <<< he).
Proof.
move=> vle vhe puh ogn0 samex.
have : sort (@edge_below _) (outgoing e) != [::].
  move: (size_sort (@edge_below _) (outgoing e)).
  case: (sort _ _)=> [ | a b] //.
  by move=>/esym/eqP/=; rewrite size_eq0 (negbTE ogn0).
move: oute'; rewrite /opening_cells.
case: (sort _ _) => [ | g1 gs] //.
rewrite /= (pvertE vle).
move=> + _.
case oca_eq : (opening_cells_aux _ _ _ _) => [w w'] /=.
move: (samex) => /eqP samex'.
move: w w' oca_eq.
elim: gs g1 => [ | g2 gs Ih] g1.
  move=> + + + out'.
  have /eqP lg1 := out' _ (mem_head _ _).
  have vg1 : valid_edge g1 (point e) by rewrite -lg1 valid_edge_left.
  rewrite /= (pvertE vg1) (pvertE vhe).
  move=> w w' [] <- <- /=.
  have -> : point e == {| p_x := p_x (point e); p_y := pvert_y (point e) g1 |}.
    rewrite pt_eqE eqxx /=.
    by have /on_pvert -> : (point e) === g1 by rewrite -lg1 left_on_edge.
  have ylthe : p_y (point e) < pvert_y (point e) he.
    by rewrite -(strict_under_pvert_y vhe).
  have /negbTE-> :
     {| p_x := p_x (point e); p_y := pvert_y (point e) he|} != point e.
    by move: ylthe; rewrite pt_eqE eqxx /= ltNge le_eqVlt negb_or=> /andP[] ->.
  rewrite /in_safe_side_left /=.
  rewrite /left_limit /= samex eqxx orbF /=.
...
rewrite /=.
move=> + + + out'.
have lg1 := out' _ (mem_head _ _).
have vg1 : valid_edge g1 (point e) by rewrite -(eqP lg1) valid_edge_left.
case oca_eq' : (opening_cells_aux _ _ _ _) => [u u'].
rewrite (pvertE vg1).
move=> w w' [] <- <- /= {w w'}.
rewrite pt_eqE /= eqxx.
have lg2 : left_pt g2 == point e.
  by apply: out'; rewrite !inE eqxx orbT.
have eong1 : point e === g1 by rewrite -(eqP lg1) left_on_edge.
have eong2 : point e === g2 by rewrite -(eqP lg2) left_on_edge.
have vg2 : valid_edge g2 (point e) by rewrite -(eqP lg2) valid_edge_left.
have vpg1 : valid_edge g1 pp by rewrite (same_x_valid _ samex').
...
have -> : P1 = false; last rewrite !andbF /=.
  rewrite {}/P1 -ltNge; apply/negP=> /andP[] /lt_trans /[apply].
  by rewrite lt_irreflexive.
apply: (Ih g2)=> //.
by move=> g gin; apply: out'; rewrite inE gin orbT.
Qed.

Lemma last_opening_cells_side_char le he pp nos lno :
  outgoing e != [::] ->
  valid_edge le (point e) ->
  valid_edge he (point e) ->
  point e <<< he ->
...
have := opening_cells_1 ogn0 vle vhe => -[fno [nos' [lno' oeq']]].
...
have ppal : (pp >>> low lno) = (p_y (point e) < p_y pp).
  ...
  by rewrite -ltNge.
rewrite ppal.
have := opening_cells_last_left_pts vle vhe ogn0 puh.
rewrite oca_eq /= => ->.
...
Lemma first_opening_cells_side_char le he pp fno nos lno :
  outgoing e != [::] ->
  valid_edge le (point e) ->
  valid_edge he (point e) ->
  point e >>> le ->
...
Lemma middle_opening_cells_side_char le he pp fno nos lno :
  outgoing e != [::] ->
  valid_edge le (point e) ->
  valid_edge he (point e) ->
  opening_cells (point e) (outgoing e) le he = rcons (fno :: nos) lno ->
...
Lemma single_opening_cell_side_char le he pp :
  valid_edge le (point e) ->
  valid_edge he (point e) ->
  point e >>> le ->
  point e <<< he ->
...
rewrite !inE !pt_eqE ppuhy andbF orbF.
move: ppae; rewrite lt_neqAle eq_sym=>/andP[] /negbTE -> _.
by rewrite andbF.
Qed.
...
have Cinv' : common_general_position_invariant bottom top s
  ...
  by rewrite /simple_step oca_eq=> /(_ _ _ lsthe lstx); apply.
...
Record edge_covered_general_position_invariant (bottom top : edge)
 ...
 { edge_covered_ec : {in processed_set, forall e,
   ...
   processed_covered : {in processed_set, forall e,
       exists2 c, c \in (state_open_seq s ++ state_closed_seq s) &
...
have cov_p1 : {in [:: e], forall e',
  exists2 c, c \in (nos ++ [:: lno]) ++ [:: close_cell (point e) op0] &
  point e' \in left_pts c}.
  move=> e'; rewrite inE => /eqP -> {e'}.
  exists lno; first by rewrite !mem_cat inE eqxx orbT.
  have := opening_cells_in vb vt oute.
  by rewrite /opening_cells oca_eq; apply; rewrite mem_rcons mem_head.
by constructor.
Qed.
...
have p_cov' : {in rcons cov_set ev, forall e, exists2 c,
   c \in state_open_seq (simple_step fc cc lc lcc le he cls lstc ev) ++
             state_closed_seq (simple_step fc cc lc lcc le he cls lstc ev) &
   point e \in left_pts c}.
  have exi := exists_cell cbtom adj (inside_box_between inbox_e).
  have [ocd [lcc_ctn [allct [allnct [flcnct [heq [leq [lein hein]]]]]]]] :=
    decomposition_main_properties oe exi.
  ...
  move=> e; rewrite mem_rcons inE=> /orP[]; last first.
    ...
    move: cin; rewrite mem_cat=> /orP[ | cin]; last first.
      exists c; rewrite // mem_cat; apply/orP; right.
      rewrite /simple_step oca_eq/state_closed_seq/= -cats1 -catA /=.
      by rewrite -cat_rcons mem_cat; rewrite cin.
(* TODO : this should be lifted to step_keeps_edge_covering_default, but
   then it should also be used in step_keeps_edge_covering. *)
    rewrite /state_open_seq/= ocd -cat_rcons !mem_cat.
    rewrite orbCA=> /orP[]cin; last first.
      exists c; rewrite // /simple_step oca_eq /state_open_seq /= mem_cat.
      by rewrite -catA -cat_rcons !mem_cat orbCA cin orbT.
    exists (close_cell (point ev) c); last first.
      by have [_ _ -> ] := close_cell_preserve_3sides (point ev) c.
    rewrite mem_cat; apply/orP; right.
    rewrite /simple_step oca_eq/state_closed_seq/= -cats1 -catA /=.
    rewrite -cat_rcons mem_cat; apply/orP; right.
    by rewrite cats1 -map_rcons map_f.
  move=> /eqP ->; exists lno.
    rewrite mem_cat /simple_step oca_eq/state_open_seq/=.
    by rewrite !mem_cat inE eqxx !orbT.
  apply: (opening_cells_in vle vhe out_e).
...
have /andP [pab put] : (point ev >>> bottom) && (point ev <<< top).
  by move: inbox_e=> /andP[].
have dif1 : {in nos ++ [:: lno], forall c, low c != high c}.
  rewrite cats1.
...
have clok : all (@closed_cell_side_limit_ok _) [:: (close_cell (point ev) op0)].
  rewrite /= andbT.
  by apply: close_cell_ok; rewrite // /contains_point underWC // underW.
...
elim=> [ | {evsq oca_eq istate invss}ev {req}future_events Ih] op cl st p_set.
  ...
  rewrite /state_open_seq/state_closed_seq/= => old_lt_fut d_e
...
have e_inv' :edge_covered_general_position_invariant bottom top s
  ...
  have := simple_step_edge_covered_general_position boxwf nocs'
      inbox_s oe e_inv.
  by rewrite /simple_step oca_eq.
(* Proving that low and high edges of every open cell are distinct. *)
have low_diff_high' :
  {in state_open_seq rstate, forall c : cell, low c != high c}.
...
have cl_safe_edge :
  ...
  have vtp : valid_edge top p.
    rewrite (same_x_valid _ samex) /valid_edge.
  ...
  by move: palc'=> /[swap] => ->.
  have ldifh : low opc != high opc by apply: d_e.
