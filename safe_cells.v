...
have [vlp vhp] : valid_edge (low c) p /\ valid_edge (high c) p.
  ...
  rewrite (eqP (allP lsx _ (last_in_not_nil dummy_pt ln0))) in ll.
  rewrite (eqP (allP rsx _ (head_in_not_nil dummy_pt rn0))) in rl.
  rewrite (eqP (allP lsx _ (head_in_not_nil dummy_pt ln0))) in lh.
  rewrite (eqP (allP rsx _ (last_in_not_nil dummy_pt rn0))) in rh.
...
have samex : all (fun p => p_x p == left_limit (start_open_cell bottom top))
  ...
  have -> : no_dup_seq_aux (pt_eqb R eq_op) W = no_dup_seq (W : seq pt).
    by apply/esym/no_dup_seq_aux_eq.
